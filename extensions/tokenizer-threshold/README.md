...
2. Implements compaction inside the engine using **agent-core `findCutPoint` semantics**: a summary wrapped as a user message (`<summary>...</summary>`) plus a contiguous verbatim tail sized by `keepRecentTokens` (default 20000) — does **not** call `delegateCompactionToRuntime`
